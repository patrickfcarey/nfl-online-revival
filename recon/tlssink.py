...
def serve(bind: str = "0.0.0.0", port: int = 443,
    ...
    # A shell backgrounds this process with SIGINT set to ignored, so a parent
    # script's `kill -INT` is silently dropped and the listener survives to
    # squat the port on the next run. SIGTERM is the signal that actually
    # arrives; route it into the same clean shutdown.
